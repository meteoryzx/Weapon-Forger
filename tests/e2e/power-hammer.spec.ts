...
for(const viewport of [{width:1280,height:900},{width:390,height:844}]) {
  test(`power: repeat, release, placement and rendered model at ${viewport.width}px`,async({page},testInfo)=>{
    ...
    await drag(page,18,0);
    const afterDrag=await inspect(page);
    expect(Math.abs(afterDrag.power.pose.x-beforeDrag.power.pose.x)+Math.abs(afterDrag.power.pose.z-beforeDrag.power.pose.z)).toBeGreaterThan(1);
    ...
    expect(Math.abs((await inspect(page)).power.pose.x)+Math.abs((await inspect(page)).power.pose.z)).toBeGreaterThan(1);
    const beforeSide=await inspect(page);
    await page.keyboard.press("d");
    const afterSide=await inspect(page);
    expect(Math.hypot(afterSide.power.pose.x-beforeSide.power.pose.x,afterSide.power.pose.z-beforeSide.power.pose.z)).toBeGreaterThan(1);
    const beforeRoll=afterSide.power.pose.roll;
    await page.keyboard.press("q");
    expect((await inspect(page)).power.pose.roll).not.toBeCloseTo(beforeRoll,6);
