...
export function grindingAsset(k:WorkshopModelKit):StationAsset {
  const root=new Group();root.name="stone-grinder";
  k.box(root,"base",[1050,85,950],[0,42.5,0],"endgrain");
  for(const x of [-230,230]){
    k.profile(root,"bearing-stand",[[-210,0],[210,0],[125,210],[65,820],[-65,820],[-125,210]],75,[x,85,0],"iron").rotation.y=Math.PI/2;
    k.cylinder(root,"bearing",60,90,[x,1000,0],"iron","x");
  }
  const wheel=k.cylinder(root,"grinding-wheel",300,100,[0,1000,0],"stone","x",64);wheel.userData.keepMesh=true;
  k.cylinder(root,"axle",25,770,[0,1000,0],"steel","x");
  k.cylinder(root,"hub",60,130,[0,1000,0],"steel","x");
  ...
  for(const angle of [0,Math.PI/3,Math.PI*2/3]){
    const dy=Math.cos(angle)*160,dz=Math.sin(angle)*160;
    k.beam(root,"flywheel-spoke",[410,780-dy,-dz],[410,780+dy,dz],16,18,"brass");
  }
  k.box(root,"treadle",[220,28,240],[370,140,240],"wood");
  k.beam(root,"pedal-link",[410,760,150],[410,150,250],18,18,"iron");
