...
export type ProceduralModelOptions = {
  materialOverrides?: Record<string, THREE.MeshStandardMaterial>;
  wireframe?: boolean;
  castShadow?: boolean;
...
function mixPalette(colors: [number, number, number][], value: number): [number, number, number] {
  if (colors.length === 0) return [138, 122, 95];
...
function createSculptMaterial(id: string, spec: SculptMaterialSpec, options: ProceduralModelOptions, denseComponent = false): THREE.MeshStandardMaterial {
  if (options.materialOverrides?.[id]) return options.materialOverrides[id]!;
