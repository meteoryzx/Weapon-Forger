...
`hammerContact(surface, x, z)` 返回当前落点的只读接触事实。`impactNormal` 是砧面坐标中的工具冲击方向；每条 `edges` 记录一条被跨越的支撑边界，并提供 `boundaryNormal`、`boundaryTangent`、从边界到载荷合力位置的 `loadPath`，以及边界上实际承载截面的 `supportPath`。矩形砧角可同时返回两条边界事实。当前事实用于锤击规则、渲染诊断和测试，不写入存档；玩家提交的姿态、落点和力度仍是确定性回放的输入。

## 版本与拒绝策略

- `stateVersion` 标记存档结构和字段含义。
- `parameterVersion` 标记规则参数和公式版本。
- 当前是 `forge-state-5` / `physics-9`。v4 自动升级结构版本但保留参数来源；首次执行新表面锤击将参数版本更新为 physics-9。旧几何和塑性历史保留，但没有跨参数版本的旧物理执行器。v2 按既有规则迁移，v3 仅允许没有旧有限切割记录的存档；不认识的版本明确拒绝。
