...
`physics-9` 保留第一轮共同机械响应、连续姿态表面锤击与有限砧边载荷路径，取消由锤面覆盖阈值切换独立条带的规则。材料响应位于 `src/forge/forge-physics.ts`，几何流动位于 `src/forge/hammer-surface.ts`，降阶弯矩与扭矩积分位于 `src/forge/hammer-bending.ts`：

- 新锤击使用三维节点点阵与有限砧面支撑，先局部压缩，再用互逆侧向积分扩展；接触事实显式给出冲击法向、支撑边界法向/切向、载荷路径与承载截面路径。矩形砧角可同时产生两条独立边界路径。法向残余曲率来自 `M(s)=sum(F_i*max(d_i-s,0))` 超过截面塑性承载力的部分，积分为转角与挠度，幅度受分配功、工具行程与单锤转角限制。未屈服尾段继承位移和切线，允许下移但不新增反向曲率。窄截面整体弯曲，宽截面采用有限切向带近似。总几何体积数值校正只调侧向位移，不改质量。旧锤击的 XPBD 求解保留兼容。
...
偏心载荷另按 `T(s)=sum(F_i*e_i)` 计算截面扭矩，e 相对完整截面中心。矩形截面的 Saint-Venant 扭转常数 J 给出近似屈服扭矩 `T_y=(sigma_y/sqrt(3))*J/min(b,h)`；超出部分积分为残余转角，横向与高度共同旋转。悬空长宽比 0.5–1 的平滑过渡连接宽短条带与细长梁截面近似，不随落点偏心程度开关。梁的弯曲/扭转共享边缘功与行程，未受扭矩尾段不再累积扭转。

新锤击每锤最大压缩系数、力度和锤面尺寸以 `HAMMER_RULES` / `FORGE_RULES` 为准；当前平锤作用面为 48 × 48 mm，压缩平滑权重为 `(1-t²)²`（范围外为零）。两次压缩/侧向积分构成近似不可压缩流动；边缘载荷使用 5 × 5 真实材料采样，覆盖包含半个采样单元边界。截面交线使用连续裁剪，避免只筛选节点造成宽度跳变。当前仍为稳定夹持、常量截面与小转角近似；未联立求解动态接触、回弹、扭转翘曲、组合屈服、完整板耦合或压缩/弯曲/扭转能量平衡。总体积误差超过 0.002% 或任一控制四面体倒置会拒绝该锤。单元局部体积未做逐格严格约束，支撑距离法、缺口截面和薄截面风险也只是可解释近似。新操作取消旧冷接触固定增量，损伤由实际应变驱动；温度按作者要求冻结，不将机械功再转换成升温。具体常数以 `HAMMER_RULES` 与 `FORGE_RULES` 为准。
