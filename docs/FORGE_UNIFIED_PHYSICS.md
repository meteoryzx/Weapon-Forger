...
`physics-2` 已完成第一轮共同机械响应落地，代码位于 `src/forge/forge-physics.ts`。当前实现保持降阶模型边界，但已经不再把锤击结果写成固定命中增量：

- 几何层继续由三维节点点阵、有限砧面和 XPBD 体积约束求解实际形变。
- 材料层使用温度软化后的屈服强度，并以累积塑性应变计算应变硬化。
- 形变使用三轴对数应变的偏差计算等效应变，再分配为弹性应变和塑性应变。
- 残余应力使用指数累积；损伤使用局部化、薄截面、支撑、温度塑性和热损伤共同驱动的指数演化。
- 机械功由屈服应力、弹塑应变和单元体积计算，记录在原始状态中，不能作为命中次数的替代计数器。

当前只把这套响应接入锤击。切割、焊合、淬火、回火和研磨必须等待这套公共载荷接口经过固定样本验证后再接入。
